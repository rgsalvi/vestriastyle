...
export async function generateTryOn(params: { person: { base64: string; mimeType: string }, flatLay: { base64: string; mimeType: string }, size?: { width: number; height: number }, strict?: boolean }): Promise<{ base64Image: string; mimeType: string }> {
  const controller = new AbortController();
  const timeout = setTimeout(() => { try { controller.abort(); } catch {} }, 60000);
  try {
